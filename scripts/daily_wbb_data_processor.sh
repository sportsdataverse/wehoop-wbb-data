...
# parquets. The team + schedule crosswalks (wbb_13-14) are Python; the player
# crosswalk (wbb_15) stays on R (live ESPN+Fox rosters),
...
for i in $(seq "${START_YEAR}" "${END_YEAR}"); do
  ...
  # don't trip over their own log output being written to a tracked file.
  {
    ...
    sdv_commit_push "WBB Data update (Start: $i End: $i)" wbb || PUSH_RC=1
  } 2>&1 | tee "$TMPLOG"

  RSCRIPT_RC=$(sed 's/RSCRIPT_RC=//' "/tmp/_rc_${i}" 2>/dev/null); rm -f "/tmp/_rc_${i}"
  cp "$TMPLOG" "$LOGFILE"
  sdv_commit_push "WBB Data log update (Start: $i End: $i)" "$LOGFILE" || PUSH_RC=1
  rm -f "$TMPLOG"
...
# (`wbb_data_build.docs --check --no-live`). Nothing regenerated them, so every
...
# ones.
( cd . && uv run python -m wbb_data_build.docs ) || echo "::warning ::docs regeneration failed"
