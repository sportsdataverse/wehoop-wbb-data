...
Everything the pipeline reports flows through one ``wbb_data_build`` logger
writing to *stdout* (not stderr), because the daily processor tees stdout into
the per-season rotating logfile (``logs/wehoop_wbb_data_logfile_{season}.log``)
...
_FORMAT = "%(asctime)s [%(levelname)s] wbb_data_build: %(message)s"
...
def get_logger() -> logging.Logger:
    ...
    logger = logging.getLogger("wbb_data_build")
