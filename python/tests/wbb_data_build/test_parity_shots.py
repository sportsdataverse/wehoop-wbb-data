"""Parity: Python shots vs the R-released parquet oracle.

Port provenance: the shots block of ``espn_wbb_01_pbp_creation.R`` (filter
``shooting_play == TRUE`` on the compiled season pbp, project the 15 shot
columns). Oracle: ``tests/fixtures/released/shots_2026.parquet`` — the
published ``espn_womens_college_basketball_shots`` asset (2026-only tag)
pre-filtered to the three 2026 fixture games (480 rows, live coordinates).
The shot rows have no unique key, so ALL columns are sort keys (total order;
duplicate rows compare fine as multisets).
"""

from pathlib import Path

import polars as pl

from tests.wbb_data_build._parity_helpers import assert_parquet_parity
from wbb_data_build.build import build_season

FX = Path(__file__).parent.parent / "fixtures"


ADDITIVE_COLS = (
    "athlete_name_1",
    "athlete_name_2",
    "team_name",
    "team_mascot",
    "team_abbrev",
)


def test_shots_parity_2026(tmp_path):
    # Production path: shots read the built pbp parquet under the same base.
    build_season("pbp", 2026, base=tmp_path, raw_root=FX / "raw")
    py = build_season("shots", 2026, base=tmp_path, raw_root=FX / "raw")
    # Additive columns (2026-07): every shot has a shooter name and team cols.
    assert py["athlete_name_1"].null_count() == 0
    assert py["team_abbrev"].null_count() == 0
    oracle = FX / "released" / "shots_2026.parquet"
    keys = list(pl.read_parquet_schema(str(oracle)))
    assert_parquet_parity(py, oracle, keys=keys, sample_cols=[], py_only_additive=ADDITIVE_COLS)
