...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/player_core/json"
...
season_athlete_ids <- function(season) {
  pb <- tryCatch(
    wehoop::load_wbb_player_box(seasons = season),
    error = function(e) {
...
build_season_player_core <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN WBB player core ({length(athlete_ids)} athletes)",
    msg_done = "Compiled {y} ESPN WBB player core!"
  )
  ...
  core <- core %>%
    ...
    wehoop:::make_wehoop_data(
      "ESPN WBB Player Core from hoopR data repository",
      Sys.time()
    )

  for (d in c("wbb/player_core", "wbb/player_core/rds", "wbb/player_core/parquet")) {
  ...
  cli::cli_progress_step(
    msg = "Updating {y} ESPN WBB Player Core GitHub Release",
    msg_done = "Updated {y} ESPN WBB Player Core GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_wbb_player_core",
    ...
    pkg_function = "sportsdataverse-data release espn_wbb_player_core",
