...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/player_season_stats/json"
gh_api_base <- "https://api.github.com/repos/sportsdataverse/wehoop-wbb-raw/contents/wbb/player_season_stats/json"
...
team_rosters_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/team_rosters/json"
team_rosters_api  <- "https://api.github.com/repos/sportsdataverse/wehoop-wbb-raw/contents/wbb/team_rosters/json"
...
# ESPN's player season stats payload uses parallel-array categories.
# Each category has labels[], names[], displayNames[], descriptions[], totals[]
# aligned by index. There are usually 3 categories: averages, totals, misc.
parse_one_category <- function(season, athlete_id, athlete_meta, category) {
  ...
  # Best-effort numeric conversion -- ESPN sometimes returns "-" / "--" for missing
...
build_season_player_stats <- function(y) {
  ...
  # Build season-scoped athlete identity lookup once; cheap (one JSON read
  # per team) and required because the ESPN player-stats endpoint omits
  # athlete identity. Without this the rows have NA names.
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN WBB player season stats ({length(athlete_ids)} athletes)",
    msg_done = "Compiled {y} ESPN WBB player season stats!"
  )
  ...
  stats <- stats %>%
    ...
    wehoop:::make_wehoop_data(
      "ESPN WBB Player Season Stats from wehoop data repository",
      Sys.time()
    )

  ifelse(
    !dir.exists(file.path("wbb/player_season_stats")),
    dir.create(file.path("wbb/player_season_stats"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/player_season_stats/rds")),
    dir.create(file.path("wbb/player_season_stats/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/player_season_stats/parquet")),
    dir.create(file.path("wbb/player_season_stats/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(
    stats,
    glue::glue("wbb/player_season_stats/rds/player_season_stats_{y}.rds")
  )
  arrow::write_parquet(
    canonicalize_ids(stats),
    glue::glue(
      "wbb/player_season_stats/parquet/player_season_stats_{y}.parquet"
    ),
    compression = "zstd",
  ...
  cli::cli_progress_step(
    msg = "Updating {y} ESPN WBB Player Season Stats GitHub Release",
    msg_done = "Updated {y} ESPN WBB Player Season Stats GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_womens_college_basketball_player_season_stats",
    pkg_function = "wehoop::load_wbb_player_stats()",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  # --- Manifest row append --------------------------------------------------
  manifest_path <- "wbb/player_season_stats/wbb_player_season_stats_in_data_repo.csv"
...
tryCatch({
  ...
  upload_wbb_manifest(
    manifest_path        = "wbb/player_season_stats/wbb_player_season_stats_in_data_repo.csv",
    release_tag          = "espn_womens_college_basketball_player_season_stats",
    file_name            = "wbb_player_season_stats_in_data_repo",
    sportsdataverse_type = "player season stats manifest",
    pkg_function         = "wehoop::load_wbb_player_stats_manifest()"
  )
}, error = function(e) {
