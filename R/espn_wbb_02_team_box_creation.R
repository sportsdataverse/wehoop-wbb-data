...
wbb_team_box_games <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN WBB Team Boxscores ({length(season_team_box_list)} games)",
    ...
    function(x) {
      tryCatch(
        expr = {
          ...
          tryCatch(
            wehoop:::helper_espn_wbb_team_box(resp),
  ...
  cli::cli_progress_step(
    msg = "Updating {y} ESPN WBB Team Boxscores GitHub Release",
  ...
  if (nrow(espn_df) > 0) {
    espn_df <- espn_df %>%
      ...
      wehoop:::make_wehoop_data(
        "ESPN WBB Team Boxscores from wehoop data repository",
        Sys.time()
      )

    ifelse(
      !dir.exists(file.path("wbb/team_box")),
      dir.create(file.path("wbb/team_box")),
      FALSE
    )

    ifelse(
      !dir.exists(file.path("wbb/team_box/rds")),
      dir.create(file.path("wbb/team_box/rds")),
      FALSE
    )
    saveRDS(espn_df, glue::glue("wbb/team_box/rds/team_box_{y}.rds"))

    ifelse(
      !dir.exists(file.path("wbb/team_box/parquet")),
      dir.create(file.path("wbb/team_box/parquet")),
      FALSE
    )
    arrow::write_parquet(
      espn_df,
