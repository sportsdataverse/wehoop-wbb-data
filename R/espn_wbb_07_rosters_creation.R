...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/team_rosters/json"
gh_api_base <- "https://api.github.com/repos/sportsdataverse/wehoop-wbb-raw/contents/wbb/team_rosters/json"
...
flatten_athletes <- function(raw) {
  ...
  # WBB sometimes wraps athletes in position-group buckets each with `items[]`
...
parse_one_team <- function(season, team_id) {
  ...
  tibble::tibble(
    ...
    full_name = purrr::map_chr(
      flat,
      ~ safe_chr(.x[["fullName"]]) %|%
        safe_chr(.x[["displayName"]])
    ),
    ...
    height = purrr::map_chr(
      flat,
      ~ safe_chr(.x[["displayHeight"]]) %|% safe_chr(.x[["height"]])
    ),
    weight = purrr::map_chr(
      flat,
      ~ safe_chr(.x[["displayWeight"]]) %|% safe_chr(.x[["weight"]])
    ),
...
# `%|%` -- a string-aware coalesce (treat NA / "" as missing).
`%|%` <- function(a, b) {
...
build_season_rosters <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN WBB rosters ({length(team_ids)} teams)",
    msg_done = "Compiled {y} ESPN WBB rosters!"
  )
  ...
  rosters <- rosters %>%
    ...
    wehoop:::make_wehoop_data(
      "ESPN WBB Rosters from wehoop data repository",
      Sys.time()
    )

  ifelse(
    !dir.exists(file.path("wbb/rosters")),
    dir.create(file.path("wbb/rosters"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/rosters/rds")),
    dir.create(file.path("wbb/rosters/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/rosters/parquet")),
    dir.create(file.path("wbb/rosters/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(rosters, glue::glue("wbb/rosters/rds/rosters_{y}.rds"))
  arrow::write_parquet(
    canonicalize_ids(rosters),
    glue::glue("wbb/rosters/parquet/rosters_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN WBB Rosters GitHub Release",
    msg_done = "Updated {y} ESPN WBB Rosters GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_womens_college_basketball_rosters",
    pkg_function = "wehoop::load_wbb_rosters()",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  # --- Manifest row append --------------------------------------------------
  manifest_path <- "wbb/rosters/wbb_rosters_in_data_repo.csv"
...
tryCatch({
  ...
  upload_wbb_manifest(
    manifest_path        = "wbb/rosters/wbb_rosters_in_data_repo.csv",
    release_tag          = "espn_womens_college_basketball_rosters",
    file_name            = "wbb_rosters_in_data_repo",
    sportsdataverse_type = "rosters manifest",
    pkg_function         = "wehoop::load_wbb_rosters_manifest()"
  )
}, error = function(e) {
