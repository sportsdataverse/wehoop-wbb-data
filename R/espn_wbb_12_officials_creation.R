...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/officials/json"
...
write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "wbb/officials/wbb_officials_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("wbb/officials")),
    dir.create(file.path("wbb/officials"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
    season = as.integer(season),
...
build_season_officials <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN WBB officials ({length(game_ids)} games)",
    msg_done = "Compiled {y} ESPN WBB officials!"
  )
  ...
  officials_df <- officials_df %>%
    ...
    wehoop:::make_wehoop_data(
      "ESPN WBB Officials from wehoop data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("wbb/officials/rds")),
    dir.create(file.path("wbb/officials/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/officials/parquet")),
    dir.create(file.path("wbb/officials/parquet"), recursive = TRUE),
    FALSE
  )
  ...
  cli::cli_progress_step(
    msg = "Updating {y} ESPN WBB Officials GitHub Release",
    msg_done = "Updated {y} ESPN WBB Officials GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_womens_college_basketball_officials",
...
tryCatch({
  ...
  upload_wbb_manifest(
    manifest_path        = "wbb/officials/wbb_officials_in_data_repo.csv",
    release_tag          = "espn_womens_college_basketball_officials",
    file_name            = "wbb_officials_in_data_repo",
    sportsdataverse_type = "officials manifest",
    pkg_function         = "wehoop::load_wbb_officials_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
