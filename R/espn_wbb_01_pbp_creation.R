...
wbb_pbp_games <- function(y) {
  ...
  sched <- wehoop:::rds_from_url(paste0(
    "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/schedules/rds/wbb_schedule_",
    y,
    ".rds"
  ))
  ifelse(
  ...
  if (length(season_pbp_list) > 0) {
    cli::cli_progress_step(
      msg = "Compiling {y} ESPN WBB pbps ({length(season_pbp_list)} games)",
      ...
      function(x) {
        tryCatch(
          expr = {
            ...
            tryCatch(
              wehoop:::helper_espn_wbb_pbp(resp),
    ...
    cli::cli_progress_step(
      msg = "Updating {y} ESPN WBB PBP GitHub Release",
  ...
  if (nrow(espn_df) > 1) {
    espn_df <- espn_df %>%
      ...
      wehoop:::make_wehoop_data(
        "ESPN WBB Play-by-Play from wehoop data repository",
        Sys.time()
      )

    ifelse(
      !dir.exists(file.path("wbb/pbp")),
      dir.create(file.path("wbb/pbp")),
      FALSE
    )
    ifelse(
      !dir.exists(file.path("wbb/pbp/rds")),
      dir.create(file.path("wbb/pbp/rds")),
      FALSE
    )
    saveRDS(espn_df, glue::glue("wbb/pbp/rds/play_by_play_{y}.rds"))

    ifelse(
      !dir.exists(file.path("wbb/pbp/parquet")),
      dir.create(file.path("wbb/pbp/parquet")),
      FALSE
    )
    arrow::write_parquet(
      espn_df,
  ...
  if (nrow(espn_df) > 0) {
    ...
    cli::cli_alert_info(
      "{length(season_pbp_list)} ESPN WBB pbps to be compiled for {y}, skipping PBP compilation"
    )
    sched$PBP <- FALSE
  }
