...
wbb_player_box_games <- function(y) {
  ...
  if (length(season_player_box_list) > 0) {
    cli::cli_progress_step(
      msg = "Compiling {y} ESPN WBB Player Boxscores ({length(season_player_box_list)} games)",
    ...
    cli::cli_progress_step(
      msg = "Updating {y} ESPN WBB Player Boxscores GitHub Release",
  ...
  if (nrow(espn_df) > 1) {
    espn_df <- espn_df %>%
      ...
      wehoop:::make_wehoop_data(
        "ESPN WBB Player Boxscores from wehoop data repository",
        Sys.time()
      )

    ifelse(
      !dir.exists(file.path("wbb/player_box")),
      dir.create(file.path("wbb/player_box")),
      FALSE
    )

    ifelse(
      !dir.exists(file.path("wbb/player_box/rds")),
      dir.create(file.path("wbb/player_box/rds")),
      FALSE
    )
    saveRDS(espn_df, glue::glue("wbb/player_box/rds/player_box_{y}.rds"))

    ifelse(
      !dir.exists(file.path("wbb/player_box/parquet")),
      dir.create(file.path("wbb/player_box/parquet")),
      FALSE
    )
    arrow::write_parquet(
      espn_df,
      glue::glue("wbb/player_box/parquet/player_box_{y}.parquet")
    )
  ...
  if (nrow(espn_df) > 0) {
    ...
    cli::cli_alert_info(
      "{length(season_player_box_list)} ESPN WBB Player Boxscores to be compiled for {y}, skipping Player Boxscores compilation"
    )
  ...
  final_sched <- final_sched %>%
    wehoop:::make_wehoop_data(
      "ESPN WBB Schedule from wehoop data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("wbb/schedules")),
    dir.create(file.path("wbb/schedules")),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/schedules/rds")),
    dir.create(file.path("wbb/schedules/rds")),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/schedules/parquet")),
    dir.create(file.path("wbb/schedules/parquet")),
    FALSE
  )
...
cli::cli_progress_step(
  msg = "Compiling ESPN WBB master schedule",
