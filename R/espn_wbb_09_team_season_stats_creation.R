...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/team_stats/json"
gh_api_base <- "https://api.github.com/repos/sportsdataverse/wehoop-wbb-raw/contents/wbb/team_stats/json"
...
write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "wbb/team_season_stats/wbb_team_season_stats_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("wbb/team_season_stats")),
    dir.create(file.path("wbb/team_season_stats"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
...
build_season_team_stats <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN WBB team season stats ({length(team_ids)} teams)",
    msg_done = "Compiled {y} ESPN WBB team season stats!"
  )
  ...
  stats <- stats %>%
    ...
    wehoop:::make_wehoop_data(
      "ESPN WBB Team Season Stats from wehoop data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("wbb/team_season_stats/rds")),
    dir.create(file.path("wbb/team_season_stats/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/team_season_stats/parquet")),
    dir.create(
      file.path("wbb/team_season_stats/parquet"),
      recursive = TRUE
    ),
    FALSE
  )
  ...
  saveRDS(
    stats,
    glue::glue("wbb/team_season_stats/rds/team_season_stats_{y}.rds")
  )
  arrow::write_parquet(
    stats,
    glue::glue(
      "wbb/team_season_stats/parquet/team_season_stats_{y}.parquet"
    ),
    compression = "zstd",
  ...
  cli::cli_progress_step(
    msg = "Updating {y} ESPN WBB Team Season Stats GitHub Release",
    msg_done = "Updated {y} ESPN WBB Team Season Stats GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_womens_college_basketball_team_season_stats",
    pkg_function = "wehoop::load_wbb_team_stats()",
...
tryCatch({
  ...
  upload_wbb_manifest(
    manifest_path        = "wbb/team_season_stats/wbb_team_season_stats_in_data_repo.csv",
    release_tag          = "espn_womens_college_basketball_team_season_stats",
    file_name            = "wbb_team_season_stats_in_data_repo",
    sportsdataverse_type = "team season stats manifest",
    pkg_function         = "wehoop::load_wbb_team_stats_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
