...
#
# These files were previously written with `fwrite(..., append = TRUE)`, a
# blind append with no dedupe, so the weekly roster refresh left 12 rows for
# season 2026 alone and anything counting rows counted RUNS rather than
# seasons. Centralised here because the same block was duplicated across ten
# creation scripts, and the duplication had already produced a scope bug: four
# copies referenced `y` while sitting inside a function whose argument was
# `season`.
#
# Args:
