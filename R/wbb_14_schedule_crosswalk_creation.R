...
build_season_schedule_crosswalk <- function(y) {
  cli::cli_progress_step(
    msg = "Compiling {y} WBB schedule crosswalk",
    msg_done = "Compiled {y} WBB schedule crosswalk!"
  )

  schedule_crosswalk <- tryCatch(
    wehoop::wbb_schedule_crosswalk(season = y),
    error = function(e) {
  ...
  saveRDS(
    schedule_crosswalk,
    glue::glue("wbb/crosswalk/rds/wbb_schedule_crosswalk_{y}.rds")
  )
  arrow::write_parquet(
    schedule_crosswalk,
    glue::glue("wbb/crosswalk/parquet/wbb_schedule_crosswalk_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} WBB Schedule Crosswalk GitHub Release",
    msg_done = "Updated {y} WBB Schedule Crosswalk GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    file_name = glue::glue("wbb_schedule_crosswalk_{y}"),
    sportsdataverse_type = "schedule crosswalk data",
    release_tag = "wbb_crosswalk",
    pkg_function = "wehoop::wbb_schedule_crosswalk()",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  # --- Manifest row append --------------------------------------------------
  manifest_path <- "wbb/crosswalk/wbb_schedule_crosswalk_in_data_repo.csv"
...
tryCatch({
  ...
  upload_wbb_manifest(
    manifest_path        = "wbb/crosswalk/wbb_schedule_crosswalk_in_data_repo.csv",
    release_tag          = "wbb_crosswalk",
    file_name            = "wbb_schedule_crosswalk_in_data_repo",
