...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/standings/json"
...
# ESPN standings payload: top-level either has `children[]` (groups/conferences),
# each carrying `standings.entries[]`, OR a single `standings.entries[]`. Each
# entry has `team{}` and `stats[]` (each stat with name, displayName, value,
# displayValue, type, abbreviation, etc.).
...
walk_groups <- function(season, node, parent_meta = NULL) {
  ...
  # Direct standings entries on this node
  ...
  # Recurse into children/groups buckets
...
write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "wbb/standings/wbb_standings_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("wbb/standings")),
    dir.create(file.path("wbb/standings"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
  ...
  if (file.exists(manifest_path)) {
    ...
    row <- data.table::rbindlist(
      list(prior, row), use.names = TRUE, fill = TRUE
    )
...
build_season_standings <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN WBB standings",
    msg_done = "Compiled {y} ESPN WBB standings!"
  )
  ...
  standings <- standings %>%
    ...
    wehoop:::make_wehoop_data(
      "ESPN WBB Standings from wehoop data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("wbb/standings/rds")),
    dir.create(file.path("wbb/standings/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/standings/parquet")),
    dir.create(file.path("wbb/standings/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(standings, glue::glue("wbb/standings/rds/standings_{y}.rds"))
  arrow::write_parquet(
    standings,
    glue::glue("wbb/standings/parquet/standings_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN WBB Standings GitHub Release",
    msg_done = "Updated {y} ESPN WBB Standings GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_womens_college_basketball_standings",
    pkg_function = "wehoop::load_wbb_standings()",
...
tryCatch({
  ...
  upload_wbb_manifest(
    manifest_path        = "wbb/standings/wbb_standings_in_data_repo.csv",
    release_tag          = "espn_womens_college_basketball_standings",
    file_name            = "wbb_standings_in_data_repo",
    sportsdataverse_type = "standings manifest",
    pkg_function         = "wehoop::load_wbb_standings_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
