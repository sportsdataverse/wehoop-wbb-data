...
build_season_team_crosswalk <- function(y) {
  cli::cli_progress_step(
    msg = "Compiling {y} WBB team crosswalk",
    msg_done = "Compiled {y} WBB team crosswalk!"
  )

  team_crosswalk <- tryCatch(
    wehoop::wbb_team_crosswalk(season = y),
    error = function(e) {
  ...
  saveRDS(
    team_crosswalk,
    glue::glue("wbb/crosswalk/rds/wbb_team_crosswalk_{y}.rds")
  )
  arrow::write_parquet(
    team_crosswalk,
    glue::glue("wbb/crosswalk/parquet/wbb_team_crosswalk_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} WBB Team Crosswalk GitHub Release",
    msg_done = "Updated {y} WBB Team Crosswalk GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    file_name = glue::glue("wbb_team_crosswalk_{y}"),
    sportsdataverse_type = "team crosswalk data",
    release_tag = "wbb_crosswalk",
    pkg_function = "wehoop::wbb_team_crosswalk()",
    file_types = c("rds", "csv", "parquet"),
    .token = Sys.getenv("GITHUB_PAT")
  )

  # --- Manifest row append --------------------------------------------------
  manifest_path <- "wbb/crosswalk/wbb_team_crosswalk_in_data_repo.csv"
  manifest_row <- tibble::tibble(
    season           = as.integer(y),
    row_count        = as.integer(nrow(team_crosswalk)),
    generated_at_utc = format(Sys.time(), tz = "UTC", usetz = TRUE),
    source_endpoint  = "wehoop::wbb_team_crosswalk()"
  )
...
tryCatch({
  ...
  upload_wbb_manifest(
    manifest_path        = "wbb/crosswalk/wbb_team_crosswalk_in_data_repo.csv",
    release_tag          = "wbb_crosswalk",
    file_name            = "wbb_team_crosswalk_in_data_repo",
