...
raw_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/game_rosters/json"
sched_base <- "https://raw.githubusercontent.com/sportsdataverse/wehoop-wbb-raw/main/wbb/schedules/rds"
...
list_game_ids <- function(season) {
  url <- glue::glue("{sched_base}/wbb_schedule_{season}.rds")
  sched <- tryCatch(
...
write_manifest_row <- function(season, row_count, source_endpoint) {
  manifest_path <- "wbb/game_rosters/wbb_game_rosters_in_data_repo.csv"
  ifelse(
    !dir.exists(file.path("wbb/game_rosters")),
    dir.create(file.path("wbb/game_rosters"), recursive = TRUE),
    FALSE
  )
  row <- tibble::tibble(
    season = as.integer(season),
...
build_season_game_rosters <- function(y) {
  ...
  cli::cli_progress_step(
    msg = "Compiling {y} ESPN WBB game rosters ({length(game_ids)} games)",
    msg_done = "Compiled {y} ESPN WBB game rosters!"
  )
  ...
  rosters_df <- rosters_df %>%
    ...
    wehoop:::make_wehoop_data(
      "ESPN WBB Game Rosters from wehoop data repository",
      Sys.time()
    )
  ...
  ifelse(
    !dir.exists(file.path("wbb/game_rosters/rds")),
    dir.create(file.path("wbb/game_rosters/rds"), recursive = TRUE),
    FALSE
  )
  ifelse(
    !dir.exists(file.path("wbb/game_rosters/parquet")),
    dir.create(file.path("wbb/game_rosters/parquet"), recursive = TRUE),
    FALSE
  )

  saveRDS(
    rosters_df,
    glue::glue("wbb/game_rosters/rds/game_rosters_{y}.rds")
  )
  arrow::write_parquet(
    rosters_df,
    glue::glue("wbb/game_rosters/parquet/game_rosters_{y}.parquet"),
    compression = "zstd",
    compression_level = 22
  )

  cli::cli_progress_step(
    msg = "Updating {y} ESPN WBB Game Rosters GitHub Release",
    msg_done = "Updated {y} ESPN WBB Game Rosters GitHub Release!"
  )
  ...
  purrr::insistently(
    ...
    release_tag = "espn_womens_college_basketball_game_rosters",
...
tryCatch({
  ...
  upload_wbb_manifest(
    manifest_path        = "wbb/game_rosters/wbb_game_rosters_in_data_repo.csv",
    release_tag          = "espn_womens_college_basketball_game_rosters",
    file_name            = "wbb_game_rosters_in_data_repo",
    sportsdataverse_type = "game rosters manifest",
    pkg_function         = "wehoop::load_wbb_game_rosters_manifest()"
  )
}, error = function(e) {
  cli::cli_alert_warning(
