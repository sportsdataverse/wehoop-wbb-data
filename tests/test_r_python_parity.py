...
(``R/espn_wbb_NN_<key>_creation.R``), Python is a build package with datasets
as ``config.REGISTRY`` rows. The numbered shims in ``python/`` bridge that:
``python/espn_wbb_NN_<key>_creation.py`` carries the SAME number as its R twin.
...
CONFIG = REPO / "python" / "wbb_data_build" / "config.py"

_R_STAGE = re.compile(r"^(?:espn_)?wbb_(?P<num>\d{2})_(?P<key>.+)_creation$")
_PY_STAGE = re.compile(r"^espn_wbb_(?P<num>\d{2})_(?P<key>.+)_creation$")
...
KNOWN_UNPAIRED: dict[str, str] = {
    "schedules": (
        "R writes it inside espn_wbb_01_pbp_creation.R (verified: saveRDS to "
        "wbb/schedules/rds/) rather than as its own numbered stage. Later stages "
        "re-write it to stamp has_* flags."
    ),
    "shots": (
        "R writes it inside espn_wbb_01_pbp_creation.R (verified: saveRDS + "
        "write_parquet to wbb/shots/) rather than as its own numbered stage."
    ),
}
...
# Numbered stages that are NOT registry datasets. This repo (unlike its
# siblings) also numbers the driver and the master roll-up, which is the
# `00`/`99` convention from sdv-pipeline-layout -- the sh driver is the 00 role
# and 99 is always last. They are real stages, just not datasets.
NON_DATASET_STAGES = {"all", "schedule_master"}
