...
from wbb_data_build.config import PKG_FUNCTION, REGISTRY
...
def test_stamp_uploads_the_four_sidecars_with_clobber():
    ...
    assert all(
        c[:3] == ["release", "upload", "espn_womens_college_basketball_game_rosters"] for c in calls
    )
...
def test_stamp_names_the_loader_for_the_tag():
    ...
    expected = PKG_FUNCTION["espn_womens_college_basketball_game_rosters"]
