...
from wbb_data_build.summary import (
    extract_errors,
    extract_warnings,
    render,
    summarize_run,
...
def test_recognises_the_legacy_r_producer_format():
    ...
    lines = ["Updated 2026 ESPN WBB play_by_play GitHub Release"]
    assert updated_tags(lines) == ["play_by_play"]


def test_both_formats_in_one_run_are_merged_and_deduped():
    lines = [
        "uploaded x.parquet -> espn_wbb_pbp (asset 1/3)",
        "uploaded y.csv -> espn_wbb_pbp (asset 2/3)",
        "Updated 2026 ESPN WBB standings GitHub Release",
    ]
    assert updated_tags(lines) == ["espn_wbb_pbp", "standings"]
...
def test_clean_run_reports_nothing(tmp_path):
    log = tmp_path / "wehoop_wbb_data_logfile_2025.log"
...
def test_exit_codes_are_collected_per_season(tmp_path):
    a = tmp_path / "wehoop_wbb_data_logfile_2025.log"
    b = tmp_path / "wehoop_wbb_data_logfile_2026.log"
...
def test_render_produces_markdown():
    out = render({"updated": ["espn_wbb_pbp"], "warnings": [], "errors": [], "seasons": {2026: 0}})
    assert "## WBB data run summary" in out
    assert "`espn_wbb_pbp`" in out
